...
function attachReactionEvent(btn) {
  btn.addEventListener('click', async () => {
    // Prevent immediate double clicking, but allow clicking again later
    if (btn.disabled) return;
    btn.disabled = true;
    setTimeout(() => { btn.disabled = false; }, 500); // Re-enable after 500ms
    ...
    // Optimistic UI update - just add +1 to reactions
